...
4. **Implement** - execute the approved tasks in order, verifying at each checkpoint. Follow `implement-feature` so implementation stays aligned with the spec and plan artifacts, using the repo's actual stack and commands. For behavior changes, implementation must pass through the test-first checkpoint before production edits.
...
- The approved plan must identify the first RED test command and the GREEN verification command before implementation begins.
