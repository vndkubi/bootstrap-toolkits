...
They should prefer summary-first hand-offs between phases so the runtime opens only the minimum phase context by default.
...
## Progressive Disclosure Phase Contract

Every `bootstrap-phase-*` skill must write a structured hand-off entry into `.github/.bootstrap-state.json`.

Use `.github/schemas/bootstrap-phase-state.schema.json` as the contract for each completed phase entry.
...
Default bootstrap hand-off mode is **summary-first**:

1. Phase N writes a concise `summary` plus a small `nextPhaseInputs` bundle.
2. Phase N+1 reads `summary` and `nextPhaseInputs` first.
3. Phase N+1 opens `detailsPath` only when the summary is insufficient, stale, or contradicted by fresh repo evidence.

This contract exists to keep the bootstrap pipeline aligned with progressive disclosure instead of re-loading every prior artifact on every phase transition.
...
- Run `.github/scripts/validate-manifest-fidelity.js` after `.github/.bootstrap-manifest.json` is drafted and before Phase 15 is declared complete. This is a must-pass gate: `removed.*` entries must not exist on disk, retained keep paths must exist, and runtime-loaded Copilot surfaces must not keep stale references to removed skills, agents, prompts, or instructions by name.
...
5. Re-run `.github/scripts/validate-manifest-fidelity.js` after deletion. Bootstrap is not complete until the validator passes against the final on-disk tree.
