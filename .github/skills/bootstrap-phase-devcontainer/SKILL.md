---
name: bootstrap-phase-devcontainer
description: "Run Phase 14 of the bootstrap pipeline: Devcontainer. Review or generate a devcontainer surface only when the target repo requires it for local workflow parity. Use when orchestrating /bootstrap-copilot one phase at a time or resuming a failed phase without loading the full monolith. Keywords: bootstrap phase 14, devcontainer, bootstrap pipeline, /bootstrap-copilot."
---

# Bootstrap Phase 14 — Devcontainer
...
- Resuming `/bootstrap-copilot` at Phase 14
- Running a single bootstrap slice with lower context cost
- Auditing or re-validating just the devcontainer step
...
- Hand-off: continue with `bootstrap-phase-cleanup-summary`.
...
- Expected Outcome: Phase 14 completes with the declared outputs and updates bootstrap state consistently.
- How to Verify: compare the retained outputs, `.github/.bootstrap-state.json` entry, and state transition against the matching Phase 14 section in `.github/skills/generate-copilot-config/SKILL.md` and `.github/schemas/bootstrap-phase-state.schema.json`.
