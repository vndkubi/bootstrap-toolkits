...
from .repo import analyze_repo, audit_context
...
class BootstrapMcpServer:
    ...
    def call_tool(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        tool = TOOLS[name]
        structured = tool.handler(arguments, self.repo_root)
        return {
            "content": [
                {
                    "type": "text",
...
def list_prompts(repo_root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(prompts_dir.glob("*.prompt.md")):
        ...
        prompts.append({"name": name, "description": f"Render {name} prompt from the bootstrap bundle."})
    return prompts
